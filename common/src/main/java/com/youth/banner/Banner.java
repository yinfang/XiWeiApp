 ...
 *
 * 使用示例：
 * 1.xml布局文件中加入 ：（可指定标题和指示器的大小，颜色，背景等属性。是否自动轮播，间隔时长等）
 * <com.youth.banner.Banner
 * android:id="@+id/banner"
 * android:layout_width="match_parent"
 * android:layout_height="0dp"
 * app:banner_default_image="@mipmap/emptylogo"
 * app:image_scale_type="fit_center"
 * app:is_auto_play="true"
 * app:layout_constraintDimensionRatio="H,2:1"/>
 * <p>
 * 2.在activity界面中使用（绑定显示数据，可指定指示器和标题显示样式，切换动画等）
 * //设置banner样式
 ...
 * //设置标题位置（当banner模式中有标题时）
 * banner.setTitleGravity(Gravity.CENTER);
...
public class Banner extends FrameLayout implements OnPageChangeListener {
    ...
    private int mIndicatorSelectedResId = R.drawable.shape_oval_red;
    private int mIndicatorUnselectedResId = R.drawable.shape_oval_gray;
    ...
    private void handleTypedArray(Context context, AttributeSet attrs) {
        ...
        mIndicatorSelectedResId = typedArray.getResourceId(R.styleable.Banner_indicator_drawable_selected, R.drawable.shape_oval_red);
        mIndicatorUnselectedResId = typedArray.getResourceId(R.styleable.Banner_indicator_drawable_unselected, R.drawable.shape_oval_gray);
    ...
    public Banner setTitleGravity(int type) {
        this.titleGravity = type;
        return this;
    }
    ...
    private void setTitleStyleUI() {
        ...
        if (titleGravity != -1)
            bannerTitle.setGravity(titleGravity);
    ...
    private void setImageList(List<?> imagesUrl) {
        ...
        for (int i = 0; i <= count + 1; i++) {
            ...
            if (imageLoader != null)
                ...
                GlideUtil.getInstance().setImage(context, (ImageView) imageView, url);
        }
    }
