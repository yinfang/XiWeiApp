...
public class MyDBHelper extends DaoMaster.OpenHelper {
    ...
    public void onUpgrade(Database db, int oldVersion, int newVersion) {
        //切记不要调用super.onUpgrade(db,oldVersion,newVersion)，否则数据会删除
        if (oldVersion < newVersion) {
            //升级数据库需要新建的表
          /*  SecurityCheckRecordeDao.createTable(db, true);
            SecurityItemsDao.createTable(db, true);
            InspectionFormDao.createTable(db, true);*/
            // 需要进行数据迁移更新的表
            DBMigrationHelper.getInstance().migrate(db, StationOndutyRecordeDao.class);
            DBMigrationHelper.getInstance().migrate(db, SecurityCheckRecordeDao.class);
            DBMigrationHelper.getInstance().migrate(db, MonitorOndutyRecordeDao.class);
            DBMigrationHelper.getInstance().migrate(db, CivilizedServiceFormDao.class);
            DBMigrationHelper.getInstance().migrate(db, ComplaintFormDao.class);
            DBMigrationHelper.getInstance().migrate(db, InspectionFormDao.class);
            DBMigrationHelper.getInstance().migrate(db, OverHighCarInDao.class);
            DBMigrationHelper.getInstance().migrate(db, OverHighCarOutDao.class);
            DBMigrationHelper.getInstance().migrate(db, SpecialFormDao.class);
            Log.d("数据库", "数据库已升级");
        }
    }
}
